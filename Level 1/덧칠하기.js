...
function solution(n, m, section) {
  // 길이가 n인 벽을 길이가 n인 배열로 생성
  let wall = Array.from(n).fill(true);

  // 다시 페인트를 칠하기로 정한 구역들을 벽에 표시
  section.map((v) => (wall[v - 1] = false));

  // 벽을 순회하면서
  let count = 0; // 롤러로 페인트 칠해야 하는 최소 횟수
  for (let i = 0; i < wall.length; i++) {
    // 다시 페인트를 칠하기로 정한 구역이라면
    if (wall[i] === false) {
      for (let j = i; j < i + m; j++) {
        wall[j] = true; // 그 구역부터 m 길이만큼 페인트를 칠함
      }
      count++; // 페인트 칠해야 하는 횟수 + 1
    }
  }

...
function solution(s, skip, index) {
  ...
  // skip 문자 제외하기
  alphabet = alphabet.filter((v) => !skip.includes(v));

  // 정답 구하기
  for (let i = 0; i < s.length; i++) {
    // 문자의 인덱스를 찾아 index 값만큼 더해주고, z를 넘어갈 경우를 고려해 alphabet의 길이로 나눈 나머지를 인덱스로 하는 문자를 찾음
    answer += alphabet[(alphabet.indexOf(s[i]) + index) % alphabet.length];
  }
